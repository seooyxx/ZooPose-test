...
joints = torch.randn((32, 17, 3))
img_metas = [{'input_center': [128, 128], 'input_scale': [1, 1], 'img_path': 'dummy_path'}]

data_samples = [
    InstanceData(
        metainfo={
            'input_center': [128, 128],
            'input_scale': [1, 1],
            'img_path': 'dummy_path',
            'input_size': [256, 256]
        },
        gt_instances=InstanceData(
            transformed_keypoints=torch.randn((17, 2)),
            keypoints_visible=torch.ones((17,))
        )
    )
]
...
with torch.no_grad():
    results = model.forward_test(img, joints, img_metas, data_samples)

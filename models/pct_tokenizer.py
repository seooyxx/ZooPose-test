...
class Tokenizer(nn.Module):
    ...
    def __init__(self,
        ...
        ## 
        self.codebook = Codebook(self.token_class_num, self.token_dim, self.decay)
        ...
        ## 
        self.decoder = TokenDecoder(tokenizer)
    ...
    def forward(self, joints, train=True):
        ...
        # codebook
        part_token_feat, encodings, encoding_indices  = self.codebook(encode_feat, device=joints.device)
        # update codebook
        e_latent_loss = self.codebook.update(encodings, encode_feat, part_token_feat) if train else None
        
        # decoder of tokenizer, recover the joints
        recoverd_joints = self.decoder(part_token_feat, bs)
...
class TokenDecoder(nn.Module):
    ...
    def forward(self, part_token_feat, bs):
...
class Codebook(nn.Module):
    def __init__(self, token_class_num, token_dim, decay):
    ...
    def forward(self, encode_feat, device):
    ...
    def update(self, encodings, encode_feat, part_token_feat):
